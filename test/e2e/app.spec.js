...
describe('End to End Tests', () => {
  ...
  it('should contain two <option> element\'s with the inner text of "Monthly" and "Quarterly"', () => {
    return pageObject
      .evaluate(() => document.querySelector('option', 'option').innerText)
      .then(headerText => {
        expect(headerText).to.not.be.null;
        expect(headerText).to.equal('Monthly', 'Quarterly');
      });
    });

  it('should contain a <button> element with the id of "calculate"', () => {
    return pageObject
      .evaluate(() => document.querySelector('button[id=calculate]'))
      .then(input => expect(input).to.exist)
  });

  it('should contain a <p> element with the id of "output"', () => {
    return pageObject
